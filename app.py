...
st.markdown("""
Predict whether an employee is at risk of attrition based on selected features.
...
if mode == "Manual Input":
    ...
    with col1:
        ...
        total_working_years = st.slider("Total Working Years", 0, 40, 10, help=feature_info["TotalWorkingYears"])
        years_with_curr_manager = st.slider("Years with Current Manager", 0, 20, 3, help=feature_info["YearsWithCurrManager"])
        years_in_current_role = st.slider("Years in Current Role", 0, 20, 3, help=feature_info["YearsInCurrentRole"])
        years_at_company = st.slider("Years at Company", 0, 40, 5, help=feature_info["YearsAtCompany"])
        monthly_income = st.number_input("Monthly Income", 1000, 20000, 5000, step=500, help=feature_info["MonthlyIncome"])
        job_level = st.slider("Job Level", 1, 5, 2, help=feature_info["JobLevel"])
        stock_option_level = st.slider("Stock Option Level", 0, 3, 1, help=feature_info["StockOptionLevel"])

    with col2:
        job_involvement = st.slider("Job Involvement", 1, 4, 3, help=feature_info["JobInvolvement"])
        job_satisfaction = st.slider("Job Satisfaction", 1, 4, 3, help=feature_info["JobSatisfaction"])
        environment_satisfaction = st.slider("Environment Satisfaction", 1, 4, 3, help=feature_info["EnvironmentSatisfaction"])
        gender = st.selectbox("Gender", ["Female", "Male"], help=feature_info["Gender"])
        overtime = st.selectbox("OverTime", ["No", "Yes"], help=feature_info["OverTime"])
        marital_status = st.selectbox("Marital Status", ["Divorced", "Married", "Single"], help=feature_info["MaritalStatus"])
        business_travel = st.selectbox("Business Travel", ["Non-Travel", "Travel_Rarely", "Travel_Frequently"], help=feature_info["BusinessTravel"])
